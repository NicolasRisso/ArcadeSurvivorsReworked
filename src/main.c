...
void Core_ProcessInput() {
    ...
    if (!player || player->type == ENTITY_TYPE_UNDEFINED || globalVariables.levelUpState.bShowLevelUp) {
        if(player && player->type == ENTITY_TYPE_UNDEFINED) globalVariables.bShowInventory = false;
        return;
    }
    ...
    if (IsGamepadAvailable(0)) {
        f32 axisX = GetGamepadAxisMovement(0, GAMEPAD_AXIS_LEFT_X), axisY = GetGamepadAxisMovement(0, GAMEPAD_AXIS_LEFT_Y);
        if (fabsf(axisX) > 0.2f) direction.x = axisX;
        if (fabsf(axisY) > 0.2f) direction.y = axisY;
    }
    if (Vector2Length(direction) > 0) player->velocity = Vector2Scale(Vector2Normalize(direction), player->character.speed);
    else player->velocity = (Vector2){0};
    // Inventory Toggle
...
void Core_UpdateGame(f32 deltaTime) {
    ...
    if (globalVariables.eventState.swarmCycleTimer >= 120) {
        globalVariables.eventState.swarmCycleTimer -= 120;
        Event_TriggerEvent(EVENT_TYPE_SWARM);
    }
    if (globalVariables.eventState.bossCycleTimer >= 210) {
        globalVariables.eventState.bossCycleTimer -= 210;
...
void Audio_PlaySoundVar(AssetSoundType type, bool bIsSpammable) {
    if (bIsSpammable) {
        i32 slot = -1;
        for (i32 i = 0; i < 3; i++) if (spammableTimers[i] <= 0) { slot = i; break; }
        if (slot == -1) return;
...
void Collision_MapBorder(Entity *entity) {
    ...
    entity->position.x = Clamp(entity->position.x, -bound, bound);
    entity->position.y = Clamp(entity->position.y, -bound, bound);
}
void PowerUp_Trigger(PowerUpType type) {
    ...
    else if (type == POWERUP_TYPE_MAGNET) {
        for (i32 i = 0; i < globalVariables.lastEntityIndex; i++) if (globalVariables.entities[i].type == ENTITY_TYPE_XP_CRYSTAL) globalVariables.entities[i].xpCrystal.bIsMagnetized = true;
    }
    else if (type == POWERUP_TYPE_DOUBLE_TROUBLE || type == POWERUP_TYPE_TIME_FREEZE) {
        globalVariables.activePowerUps[type] = (ActivePowerUp){ .type = type, .remainingTime = 15, .bIsActive = true };
    }
}
void Instant_Trigger(InstantDropType type) {
    Entity* player = Global_GetPlayer();
    f32 healRatio = type == INSTANT_DROP_TYPE_LIFE ? 0.2f : (type == INSTANT_DROP_TYPE_BIG_LIFE ? 0.5f : 0);
    if (healRatio > 0) {
        player->character.health = fminf(player->character.maxHealth, player->character.health + player->character.maxHealth * healRatio);
...
void Drop_ProcessPickUp() {
    ...
    for (i32 i = 0; i < globalVariables.lastEntityIndex; i++) {
        ...
        if (drop->type == ENTITY_TYPE_DROP && CheckCollisionCircles(player->position, player->radius, drop->position, drop->radius)) {
            if (drop->drop.dropType == DROP_TYPE_POWERUP) PowerUp_Trigger(drop->drop.powerUpType);
            else Instant_Trigger(drop->drop.instantDropType);
            Global_DestroyEntity(i--);
        }
    }
}
...
Entity Enemy_GenerateEnemy(EnemyType enemyType) {
    const struct { f32 health, speed, xp, damage, scale, radius; } typeData[] = { {0, 0, 0, 0, 0, 0}, {15, 150, 10, 10, 0.75f, 30}, {10, 225, 15, 5, 0.5f, 30}, {115, 80, 50, 25, 1.5f, 60}, {3500, 120, 5000, 50, 3.0f, 120} };
    ...
    for (i32 entityIndex = 0; entityIndex < globalVariables.lastEntityIndex; entityIndex++) {
        ...
        if (!bTimeFrozen) {
            ...
            if (current->velocity.x != 0) {
                if (current->visualType == VISUAL_TYPE_ANIMATED_STATIC_SPRITE) current->animatedStaticSprite.flipX = current->velocity.x < 0;
                else current->sprite.flipX = current->velocity.x < 0; }
        ...
        for (i32 otherIndex = 0; otherIndex < globalVariables.lastEntityIndex; otherIndex++) {
            Entity *other = &globalVariables.entities[otherIndex];
            if (entityIndex == otherIndex || other->type != ENTITY_TYPE_ENEMY) continue;
        ...
        if (player->character.invulnerableTimer <= 0 && Vector2Distance(current->position, player->position) < (current->radius + player->radius)) {
            player->character.health -= current->enemyCharacter.damage;
            player->character.invulnerableTimer = player->character.flashTimer = 0.5f; 
...
void HUD_DrawBar(i32 posX, i32 posY, i32 width, i32 height, f32 percentage, Color color) {
    ...
    DrawRectangle(posX, posY, (i32)(width * Clamp(percentage, 0, 1)), height, color);
...
void HUD_Draw() {
    ...
    if (globalVariables.levelUpState.bShowLevelUp) {
        xpPercentage = 1;
        xpColor = ColorFromHSV((f32)((i32)(GetTime() * 300) % 360), 0.8f, 0.9f);
    }
    HUD_DrawBar(0, 0, SCREEN_WIDTH, 30, xpPercentage, xpColor);
    HUD_DrawTextC(TextFormat("LEVEL %d", globalVariables.playerStats.level), 0, 5, SCREEN_WIDTH, 20, WHITE, false);
    Entity* player = Global_GetPlayer();
    HUD_DrawBar(20, 50, 350, 35, player->character.health / player->character.maxHealth, RED);
    ...
    DrawText(timerText, timerX, 50, 40, WHITE);
    HUD_DrawPowerUps();
...
void HUD_DrawInventory() {
    ...
    for (i32 index = 0; index < MAX_WEAPON_CAPACITY; index++) if (globalVariables.inventory.weaponDatas[index].level > 0) {
        DrawText(TextFormat("Lv.%d %s", globalVariables.inventory.weaponDatas[index].level, WeaponNames[globalVariables.inventory.weaponDatas[index].weaponType]), padding + 20, currentY, 32, WHITE);
        currentY += 40;
    }
    DrawText("RELICS", padding, currentY += 30, 30, GRAY);
    currentY += 45;
    for (i32 index = 0; index < MAX_RELIC_CAPACITY; index++) if (globalVariables.inventory.relicDatas[index].level > 0) {
        DrawText(TextFormat("Lv.%d %s", globalVariables.inventory.relicDatas[index].level, RelicNames[globalVariables.inventory.relicDatas[index].relicType]), padding + 20, currentY, 32, WHITE);
        currentY += 40;
    }
    ...
    for (i32 index = 0; index < 7; index++) {
        DrawText(statTextStrings[index], rightPositionX, currentY, 32, WHITE);
        currentY += 45;
    }
}
void HUD_DrawLevelUp() {
    static f32 axisTimer = 0; i32 choiceIndex = -1; bool bSelected = false;
    if (IsKeyPressed(KEY_ONE)) { choiceIndex = 0; bSelected = true; }
    if (IsKeyPressed(KEY_TWO)) { choiceIndex = 1; bSelected = true; }
    if (IsKeyPressed(KEY_THREE)) { choiceIndex = 2; bSelected = true; }
...
void HUD_GenerateLevelUpOptions() {
    ...
    for (i32 typeIndex = 1; typeIndex <= 2; typeIndex++) {
        ...
        for (i32 upgradeType = 1; upgradeType < maxTypesCount; upgradeType++) {
            // Hey Shiki, I know this line in particular looks insanelly ugly, but hey, it is just one command inside another! So it is technically doable in one line :D
...
SpawnerData Spawner_GenerateSpawnerData() {
    ...
    SpawnDefinition defs[] = { {1, 1, { 1,  1}, { 800, 1000}, 100,  0}, {2, 2, { 5,  8}, { 900, 1100},  30,  0}, {1, 4, {10, 15}, {1000, 1200},  20,  0}, {3, 1, { 1,  1}, {1100, 1300},  10, 15}, {2, 3, { 5, 10}, { 900, 1100},  25,  0}, {1, 2, {11, 18}, { 900, 1100},  35,  0}, {1, 3, {12, 19}, { 900, 1100},  35,  0}, {2, 2, {15, 25}, { 800, 1000},  40,  5}, {2, 4, {20, 30}, {1000, 1200},  30, 10}, {3, 2, { 2,  3}, {1100, 1300},  15, 25}, {2, 4, {30, 45}, { 700,  900},  25, 20}, {1, 3, {40, 60}, {1000, 1200},  20, 35}, {3, 2, { 5,  8}, {1200, 1400},  10, 45}, {2, 2, {50, 80}, { 900, 1100},  30, 60} };
    for (int i = 0; i < MAX_SPAWN_DEFINITION; i++) data.spawnsDefinitions[i] = defs[i];
    return data;
}

void Spawner_ProcessSpawnLogic(f32 deltaTime) {
    Entity* player = Global_GetPlayer();
    if (player->type == ENTITY_TYPE_UNDEFINED || (globalVariables.spawnerData.spawnTimer -= deltaTime) > 0) return;
    SpawnerData* data = &globalVariables.spawnerData;
    data->currentDifficulty = (globalVariables.gameTimer / 60.0f) * 10.0f;
    f32 multiplier = (globalVariables.eventState.activeEventType == EVENT_TYPE_SWARM ? 3.0f : (globalVariables.eventState.activeEventType == EVENT_TYPE_BOSS  ? 1.5f : 1.0f));
    data->spawnTimer = (data->delayBetweenSpawns / powf(1.15f, floorf(globalVariables.gameTimer / 60.0f))) / multiplier;
    i32 totalWeight = 0;
    for (int i = 0; i < MAX_SPAWN_DEFINITION; i++) if (data->currentDifficulty >= data->spawnsDefinitions[i].Difficulty) totalWeight += data->spawnsDefinitions[i].chanceToSpawn;
    ...
    Vector2 basePos = { player->position.x + cosf(angle) * dist, player->position.y + sinf(angle) * dist };
...
static void Action_CrystalWand(WeaponData* weaponData, WeaponDefinition* weaponDefinition, Entity* player, f32 deltaTime, f32 attackSpeed) {
    if (weaponData->burstRemaining > 0 && (weaponData->burstTimer -= deltaTime) <= 0) {
        ...
        if (targetId != -1) {
            Entity projectile = Projectile_Spawn(PROJECTILE_TYPE_CRYSTAL_SHARD, player->position, Vector2Scale(Vector2Normalize(Vector2Subtract(globalVariables.entities[targetId].position, player->position)), 600.0f), weaponDefinition->damage, 3.0f, weaponDefinition->crystal.penetration);
            Global_AddEntity(&projectile);
            weaponData->burstRemaining--;
            weaponData->burstTimer = 0.1f;
        }
    }
    if ((weaponData->attackTimer -= deltaTime) <= 0) {
        weaponData->attackTimer = weaponDefinition->delayBetweenAttacks / attackSpeed;
        weaponData->burstRemaining = weaponDefinition->projectileAmount;
        weaponData->burstTimer = 0.0f;
    }
}
static void Action_FireballRing(WeaponData* weaponData, WeaponDefinition* weaponDefinition, Entity* player, f32 deltaTime, f32 attackSpeed) {
    if ((weaponData->attackTimer -= deltaTime) <= 0) {
        ...
        for (i32 directionIndex = 0; directionIndex < 4; directionIndex++) for (i32 amountIndex = 0; amountIndex < weaponDefinition->projectileAmount; amountIndex++) {
            ...
            projectile.projectile.explosive.explosionRadius = weaponDefinition->fireball.explosionRadius;
            projectile.projectile.explosive.explosionDamageMultiplier = weaponDefinition->fireball.explosionDamageMultipler;
            Global_AddEntity(&projectile);
        }
    }
}
static void Action_BombShoes(WeaponData* weaponData, WeaponDefinition* weaponDefinition, Entity* player, f32 deltaTime, f32 attackSpeed) {
    if ((weaponData->attackTimer -= deltaTime) <= 0) {
        ...
        projectile.projectile.timer = weaponDefinition->bombShoes.delayToExplode; projectile.radius = weaponDefinition->bombShoes.explosionRadius;
        Global_AddEntity(&projectile);
    }
}
static void Action_NatureSpikes(WeaponData* weaponData, WeaponDefinition* weaponDefinition, Entity* player, f32 deltaTime, f32 attackSpeed) {
    if ((weaponData->attackTimer -= deltaTime) <= 0) {
        ...
        for (i32 amountIndex = 0; amountIndex < weaponDefinition->projectileAmount; amountIndex++) {
            ...
            if (candidateCount > 0) {
                Entity projectile = Projectile_Spawn(PROJECTILE_TYPE_NATURE_SPIKE, globalVariables.entities[candidates[GetRandomValue(0, candidateCount - 1)]].position, (Vector2){0}, weaponDefinition->damage, weaponDefinition->natureSpikes.spikeDuration, weaponDefinition->natureSpikes.spikeMaxDamage);
...
static void Action_DeathAura(WeaponData* weaponData, WeaponDefinition* weaponDefinition, Entity* player, f32 deltaTime, f32 attackSpeed) {
    if ((weaponData->attackTimer -= deltaTime) <= 0) {
        ...
        if (globalVariables.deathAuraIndex >= globalVariables.lastEntityIndex) { Entity projectile = Projectile_Spawn(PROJECTILE_TYPE_DEATH_AURA, player->position, (Vector2){0}, weaponDefinition->damage, 99999.0f, 255); projectile.radius = weaponDefinition->deathAura.size * globalVariables.playerStats.sizeMultiplier; projectile.projectile.timer = 0.0f; Global_AddEntity(&projectile); }
            ...
            aura->radius = weaponDefinition->deathAura.size * globalVariables.playerStats.sizeMultiplier;
            aura->projectile.damage = weaponDefinition->damage * globalVariables.playerStats.damageMultiplier;
        }
    }
}
void Weapon_Init() {
...
void Weapon_ProcessAttack(f32 deltaTime) {
    ...
    for (i32 weaponIndex = 0; weaponIndex < MAX_WEAPON_CAPACITY; weaponIndex++) {
        WeaponData* weaponData = &globalVariables.inventory.weaponDatas[weaponIndex];
        if (weaponData->level == 0) continue;
